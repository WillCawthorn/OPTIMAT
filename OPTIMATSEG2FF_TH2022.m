%Apply NIfTI ROI to ff.mat run from desktop in office
function UseMANROI=OPTIMATSEG2FF_TH2022
%select ROI file/s to process
[FileRIN,FolderRIN] = uigetfile('x:\MR_multi-echo_having_Dixon_data_All\*.nii','Select NIfTI ROI file(s)','MultiSelect', 'on');%test
%select folder containg fat fractions
FolderFFIN = uigetdir('x:\MR_multi-echo_having_Dixon_data_All\','Select folder containg fat fractions');
% creat output folders  for CSV, ROI(nifti) and FF(nifti) 
FolderOUT =FolderRIN(1:end-1);
warning('off', 'MATLAB:MKDIR:DirectoryExists');
FolderOUTcsv = strcat(FolderOUT,'CSV1');
FolderOUTff = strcat(FolderOUT,'FFnifti1');
FolderOUTroi = strcat(FolderOUT,'ROInifti1');
mkdir(FolderOUTcsv);
mkdir(FolderOUTff);
mkdir(FolderOUTroi);
...
for i =1:NF
   %read roi name 
    ...
    if CheckREG == 'T' 
        ...
        if CheckREG == 'S'
            FFREG ='_Back';
        elseif CheckREG == 'H'|CheckREG == 'N'|CheckREG == 'E'|CheckREG == 'T'
            FFREG ='_Hip';
        elseif CheckREG == 'D'
            FFREG ='_Leg';
        end
