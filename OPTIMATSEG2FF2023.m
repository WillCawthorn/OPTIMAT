%Apply NIfTI ROI to ff.mat run from IAPC2 remotely
function UseMANROI=OPTIMATSEG2FF2023
%select ROI file/s to process
[FileRIN,FolderRIN] = uigetfile('E:\OPTIMAT\UKBB_Sept2022\*.nii','Select NIfTI ROI file(s)','MultiSelect', 'on');%test
%select folder containg fat fractions
FolderFFIN = uigetdir('E:\OPTIMAT\UKBB_Sept2022\','Select folder containg fat fractions');
% creat output folders  for CSV, ROI(nifti) and FF(nifti) 
FolderOUT =FolderRIN(1:end-1);
warning('off', 'MATLAB:MKDIR:DirectoryExists');
FolderOUTcsv = strcat(FolderOUT,'CSVF');
FolderOUTff = strcat(FolderOUT,'FFniftiF');
FolderOUTroi = strcat(FolderOUT,'ROIniftiF');
mkdir(FolderOUTcsv);
mkdir(FolderOUTff);
mkdir(FolderOUTroi);
...
for i =1:NF
   %load ROI
    ...
    if CheckREG == 'S'
        FFREG ='_Back';
    elseif CheckREG == 'T'|CheckREG == 'N'|CheckREG == 'E'
        FFREG ='_Hip';
    elseif CheckREG == 'D'
        FFREG ='_Leg';
    end
